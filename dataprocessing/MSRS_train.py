...
def is_low_contrast(image, fraction_threshold=0.1, lower_percentile=10,
    ...
    ratio = (limits[1] - limits[0]) / (limits[1] + 0.00000001)
...
IR_files = sorted(get_img_file(r"..."))
VIS_files   = sorted(get_img_file(r"..."))

assert len(IR_files) == len(VIS_files)
#os.mkdir(os.path.join('./data', data_name+"_"+str(img_size)+"_"+str(stride)+'.h5'))
...
for i in tqdm(range(len(IR_files))):
        I_VIS = cv2.imread(VIS_files[i]).astype(np.float32)[:, :, ::-1].transpose(2, 0, 1) # [3, H, W] Uint8->float32
        I_VIS = rgb2y(I_VIS) # [1, H, W] Float32
        I_IR = cv2.imread(IR_files[i], cv2.IMREAD_GRAYSCALE).astype(np.float32)[None, :, :]  # [1, H, W] Float32
